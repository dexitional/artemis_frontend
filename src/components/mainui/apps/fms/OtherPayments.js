...
import { generateIndexNo, deletePayment, fetchPaymentFMS, fetchPaymentsFMS, loadFMSHelpers, postPaymentFMS, fetchOtherPaymentsFMS, moveToFees } from '../../../../store/utils/ssoApi';
...
const OtherPayments = ({view,data,recid}) => {
   
   const dispatch = useDispatch()
   const { sso } = useSelector(state => state)
   const title = () => {
     switch(view){
       case 'list': return 'OTHER PAYMENTS';
...
const List = () => {
   ...
   const [ otherpayments, setPayments ] = useState([])
   ...
   const restorePayments = () => {
      sso.databox.otherpayments && setPayments([...sso.databox.otherpayments]);
   }
   ...
   const delPayment = async (id) => {
      ...
      if(cm) {
        ...
        if(resp.success){
           const ss = otherpayments.filter(s => s.id != id)
           setPayments([...ss ])
  ...
  const editPayment = async (id) => {
    history.push(`/app/fms?mod=othertrans&view=edit&recid=${id}`)
  }
  ...
  const movePaymentToFees = async (id) => {
      const cm = window.confirm("CONVERT PAYMENT TO ACADEMIC FEES !")
      if(cm){
        const resp = await moveToFees(id)
        console.log(resp);
        if(resp.success){
          const ss = otherpayments.filter(s => s.id != id)
          setPayments([...ss ])
          dispatch(updateAlert({show:true,message:`PAYMENT CONVERTED TO ACADEMIC FEES !`,type:'success'}))
        }else{
          dispatch(updateAlert({show:true,message:`ACTION FAILED!`,type:'error'}))
        }
      }
   ...
   const fetchPayments = async () => {
      ...
      const res = await fetchOtherPaymentsFMS(query);
      if(res.success){
        setIsLoading(false)
  ...
  const handleClose = () => {
   ...
   useEffect(() => {
     dispatch(updateDatabox({otherpayments}));
   },[otherpayments])
                        ...
                        <tbody>
                          ...
                          <tr className="data-item odd" role="row">
                            ...
                            <td className="data-col center">
                                ...
                                {row.tag ? parse(`<small style="color:#b76117;font-weight:bolder"> SETTLEMENT: ${row.tag} </small>`) : null }
                            </td>
                            ...
                            <td className="data-col">
                                <>
                                  ...
                                  <Menu id={`basic-menu${row.id}`} anchorEl={anchorEl} open={pid && pid == row.id} onClose={handleClose} variant="outlined" MenuListProps={{'aria-labelledby': `basic-button${row.id}`}}>
                                    ...
                                    <MenuItem onClick={()=> movePaymentToFees(row.id)}>CONVERT TO FEES</MenuItem>
                                    {/*
