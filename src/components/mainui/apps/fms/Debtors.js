...
const List = () => {
  ...
  const restoreStudentData = () => {
    sso.databox.debtors && setDebtors([...sso.databox.debtors]);
  }
  ...
  const fetchStudentData = async () => {
      ...
      const res = await fetchStudentDebtorsFMS(query);
      if(res.success){
        setIsLoading(false)
        setDebtors([...res.data.data]);// Page Data
  ...
  const onSubmitSearch = async (e) => {
   ...
   useEffect(() => {
     dispatch(updateDatabox({debtors}));
   },[debtors])
