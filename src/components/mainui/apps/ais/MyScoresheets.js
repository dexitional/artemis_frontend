...
const MyScoresheets = ({view,data,recid}) => {
   ...
   const title = () => {
     switch(view){
       case 'list': return 'MY SCOREHEETS';
...
const List = () => {
   ...
   const exportSheet = async (id) => {
    ...
    const fetchSheetData = async () => {
      ...
      const res = await fetchMySheetDataAIS(query);
      if(res.success){
          setIsLoading(false)
                          ...
                          <tr className="data-item odd" role="row">
                            ...
                            <td className="data-col">
                                <small className="lead tnx-id"> 
                                   <button onClick={(e) => viewMiniProfile(user && user.user.staff_no)} className="btn btn-xs text-white" style={{minWidth:'auto',lineHeight:'10px',backgroundColor:'#b76117'}}><b>{user && user.user.staff_no}</b></button>
                                </small>
                            </td>
...
const ScoreForm = ({recid}) => {
  ...
  const onSubmit = async data => {
      ...
      if(res.success){
        ...
        setTimeout(() => {
            history.push(`/app/ais?mod=scoresheets&view=assess&recid=${recid}`)
        },2000)
      } else{
