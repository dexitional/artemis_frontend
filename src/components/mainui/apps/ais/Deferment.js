...
const Deferment = ({view,data,recid}) => {
   const title = () => {
     switch(view){
       case 'list': return 'DEFERMENTS';
	   ...
	   <h3 className="sub-head bg-blueblack"> 
           ...
           { view == 'list' ?
            <div className="d-inline-block print-btn">
                <Link to="/app/ais?mod=deferment&view=add" className="btn btn-light-alt btn-sm btn-icon"><em className="fa fa-sm fa-plus"></em>&nbsp;&nbsp;<b>ADD REQUEST</b></Link>
            </div> : null
           }
	   </h3>
        {content()}
...
const List = () => {
   ...
   const fetchDeferData = async () => {
      const res = await fetchDefer();
      if(res.success) setDeferment([...res.data]);
   }
   ...
   useEffect(() => {
     dispatch(updateDatabox({deferment}));
   },[deferment])
